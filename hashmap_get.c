...
/*   Updated: 2025/04/13 20:46:59 by bgebetsb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline t_line	__attribute__((always_inline)) tree_lookup_hash(t_tree *tree, t_line key, size_t hash)
{
	t_tree	*cur;
	t_line	ret;
	...
	while (cur)
	{
		...
		{
			int diff = ft_strcmp(key.raw, cur->key.raw);
			if (diff == 0)
				return (cur->value);
			else if (diff < 0)
				cur = cur->left;
